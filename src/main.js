...
// Userscript injection removed - using Equicord Web extension instead
...
// Load browser extensions (Equicord Web, etc.)
async function loadBrowserExtensions() {
  try {
    ...
    const equicordWebPath = path.join(extensionsDir, 'equicord');
    const tampermonkeyPath = path.join(extensionsDir, 'tampermonkey');
    
    // Priority 1: Try to load Equicord Web extension (preferred)
    if (fs.existsSync(equicordWebPath)) {
      try {
        await session.defaultSession.extensions.loadExtension(equicordWebPath, {
          allowFileAccess: true
        });
        console.log('✅ Equicord Web extension loaded successfully');
        return; // Success, no need to try other extensions
      } catch (error) {
        console.log('⚠️ Failed to load Equicord Web extension:', error.message);
      }
    } else {
      console.log('⚠️ Equicord Web extension not found at:', equicordWebPath);
      console.log('💡 Run: npm run get-equicord-web for instructions');
    }
    
    // Fallback: Try Tampermonkey if Equicord Web not available
...
async function loadExtensionsForSession(ses) {
  try {
    ...
    // Priority 1: Equicord Web
    if (fs.existsSync(equicordWebPath)) {
      try {
        await ses.extensions.loadExtension(equicordWebPath, {
          allowFileAccess: true
        });
        console.log('✅ Equicord Web loaded for session');
        return; // Success, no need for fallback
      } catch (error) {
        console.log('⚠️ Failed to load Equicord Web for session:', error.message);
    ...
    view.webContents.executeJavaScript(`
      ...
      window.addEventListener('error', (event) => {
        if (event.error && event.error.message && event.error.message.includes('proxyLazy')) {
          console.warn('🔧 Equicord compatibility issue detected:', event.error.message);
      ...
      window.addEventListener('unhandledrejection', (event) => {
        if (event.reason && event.reason.message && event.reason.message.includes('proxyLazy')) {
          console.warn('🔧 Equicord promise rejection handled:', event.reason.message);
...
// Userscript reloading removed - using Equicord Web extension instead
